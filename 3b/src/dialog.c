...
int D_Find_Key1(struct Table *table){
    ...
    if (ks1){
        print_item(load_item(table, ks1->item_offset));
    } else printf("Element not found");
...
int D_Find_Key2(struct Table *table){
    ...
    struct Node2 *node = find_in_ks2_iterator(table, key2);



    if (node){
        do {
            print_item(load_item(table, node->item_offset));
            node = find_in_ks2_iterator(table, -1);
        } while (node);
    } else printf("Element not found");
...
int D_Find_Key2Version(struct Table *table){
    ...
    struct Node2 *node = find_in_ks2_version(table, key2, version);
...
int D_Delete_Key1(struct Table *table){
    ...
    struct Item *item = load_item(table, find_in_ks1(table, key1)->item_offset);
...
int D_Delete_Key2Last(struct Table *table){
    ...
    if (node){
        delete_item(table, load_item(table, node->item_offset));
    } else printf("Element not found");
...
//    } while (1);
    ...
    int status = insert_table(table, key1, parent, key2, info);

    if (status == -1) printf("Table full.\n");
    else if (status == -2) printf("Key1 is already present.\n");
    else if (status == -3) printf("Parent not found.\n");
